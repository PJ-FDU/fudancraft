...
* `GridPath optimizePath(const GridPath& orig_paht) const;GridPath optimizePath(const GridPath& orig_paht) const;`优化路径函数
...
首先，UnitManager会读取瓦片地图中对象层“InitialUnits”的信息，用于创建本客户端的初始单位。每个客户端的UnitManager